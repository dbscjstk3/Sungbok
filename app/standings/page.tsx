...
function computePersonalDetail(playerId: string, players: Player[], rounds: Round[], stats: PlayerStat[]): PersonalDetail | null {
  ...
  if (teammateCount.size > 0) {
    const [tmId, tmStat] = [...teammateCount.entries()].sort((a, b) => b[1].games - a[1].games)[0]
          ...
          <div className="w-full sm:max-w-md rounded-t-2xl sm:rounded-2xl p-6 pb-8"
            ...
            {personalDetail.topTeammate && (
              <div className="rounded-xl px-4 py-3" style={{ backgroundColor: '#DEE0E2' }}>
                <p className="text-xs mb-1" style={{ opacity: 0.5 }}>최다 파트너</p>
                <p className="text-sm font-bold">
                  {personalDetail.topTeammate.player.real_name}
                  <span className="font-normal ml-2" style={{ opacity: 0.5 }}>
                    {personalDetail.topTeammate.games}판 함께 · {personalDetail.topTeammate.wins}승
                  </span>
                </p>
              </div>
            )}
        ...
        {!loading && stats.length > 0 && (
          ...
          {duoStats.length > 0 && (
            <div className="mt-12">
              ...
              <p className="text-sm mb-6" style={{ opacity: 0.5 }}>3판 이상 함께한 조합만 표시됩니다.</p>
              <div className="rounded-2xl overflow-hidden" style={{ backgroundColor: '#DEE0E2' }}>
                <table className="w-full text-xs sm:text-sm">
                  <thead>
                    <tr style={{ borderBottom: '1px solid #ECEEF0' }}>
                      <th className="text-center px-2 sm:px-4 py-3 sm:py-4 font-semibold w-8 sm:w-12" style={{ opacity: 0.5 }}>#</th>
                      <th className="text-left px-2 sm:px-4 py-3 sm:py-4 font-semibold" style={{ opacity: 0.5 }}>조합</th>
                      <th className="text-center px-2 sm:px-4 py-3 sm:py-4 font-semibold" style={{ opacity: 0.5 }}>판수</th>
                  ...
                  <tbody>
                    {duoStats.slice(0, 10).map((d, i) => (
                      <tr key={`${d.player1.id}-${d.player2.id}`} style={{ borderTop: '1px solid #ECEEF0' }}>
                        <td className="text-center px-2 sm:px-4 py-2.5 sm:py-4 font-medium" style={{ opacity: 0.35 }}>{i + 1}</td>
                        <td className="px-2 sm:px-4 py-2.5 sm:py-4 font-bold">
                          {d.player1.real_name} + {d.player2.real_name}
                        </td>
                        <td className="text-center px-2 sm:px-4 py-2.5 sm:py-4">{d.games}</td>
                        <td className="text-center px-2 sm:px-4 py-2.5 sm:py-4 font-bold">{d.wins}</td>
                        <td className="text-center px-2 sm:px-4 py-2.5 sm:py-4 font-bold"
